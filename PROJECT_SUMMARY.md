...
## تدقيق ما قبل التسليم — 2026-07-03

تدقيق نهائي شامل قبل التسليم على 3 مراحل (تدقيق سكوني، محاكاة 10 سنوات + تحقق آلي، بناء). التفاصيل الكاملة في `AUDIT_FINDINGS.md`.

**عدد الأخطاء المكتشفة والمصحَّحة: 6** (4 منطقية حرجة/متوسطة، 1 حماية دفاعية، 1 هوية تعبئة) + إضافة معاينة معزولة (F-06).

### الأخطاء المنطقية (مصحَّحة ومُتحقَّقة بالمحاكاة)

- **F-01 (حرج) — ازدواج احتساب الدفعة الأولية:** `addMaintenanceInvoice`/`addDirectSaleInvoice`/`addSupplierInvoice` كانت تُدرِج الترويسة بـ `amount_paid = calcPaid(payments)` **ثم** يزيدها `insertPayments` مرة أخرى — فتُحتسب الدفعة الأولية مرتين (المورد وصل لمتبقٍّ سالب −40). **الإصلاح:** الترويسة تُدرَج بـ `amount_paid=0` / `amount_remaining=total`، و`insertPayments` (المصدر الوحيد المشترك مع التسليم/الدفعات) يراكمها مرة واحدة.
- **F-02 (متوسط) — رصيد الصندوق التراكمي يتقادم بعد الحذف/التعديل:** حذف فاتورة/مصروف/راتب كان يزيل قيود `cash_ledger` دون إعادة حساب `balance_after` للصفوف اللاحقة. **الإصلاح:** `recomputeLedgerBalances()` مركزية في `ledger.ts` تُستدعى بعد كل حذف يمسّ الصندوق، وتعديل المصروف/الراتب صار **تحديثاً في مكان القيد** بدل حذفه وإعادة إدراجه في نهاية السجل.
- **F-03 (متوسط) — تعديل بنود فاتورة مدفوعة إلى إجمالي أقل من المدفوع:** كان ينتج `amount_remaining` سالباً بصمت. **الإصلاح:** حماية في مسارات التعديل الثلاثة ترفض هبوط الإجمالي تحت (المدفوع + خصم التسوية). (+ **F-03ب:** حماية دفاعية عند الإنشاء ترفض دفعة أولية تتجاوز الإجمالي.)
- **F-04 (حرج) — إعادة حساب المتبقّي عند التعديل تتجاهل خصم التسوية:** كانت تحسب `remaining = total − paid` متجاهلةً خصم التسوية السابق فيتضخّم المتبقّي (يُطالَب الزبون بمبلغ سبق إسقاطه)، وكُسِر الثابت `total = paid + remaining + settlement`. **الإصلاح:** `remaining = total − paid − Σsettlement` في المسارات الثلاثة.

### هوية التطبيق (طُبِّقت بقرار المالك)

- **F-05 — هوية التطبيق:** ضُبِطت من placeholder إلى `appId="com.greenline.garage"` / `productName="GreenLineGarage"` (لاتيني لسلامة أسماء الملفات) / `version="1.0.0"`. **تنبيه:** يجب عدم تغيير `productName` بعد أول تثبيت لدى الزبون (يتيّم مجلد بيانات المستخدم `userData`).

### أدوات الاختبار + المعاينة المعزولة (`scripts/`)

`simulate-history.ts` (توليد ~10 سنوات عبر قنوات IPC الحقيقية) + `verify-integrity.ts` (7 فحوص سلامة تقارن التقارير بالجداول الخام) + بنية تشغيل (`electron-stub.mjs`/`hooks.mjs`/`register.mjs`) تختبر الـ backend الحقيقي خارج Electron. **خارج مسار الإنتاج تماماً** (لا تُحزَّم، لا تمسّ قاعدة المستخدم، لا تُستدعى من `npm run dev`/`build`).

**F-06 — معاينة معزولة:** أُضيف لـ `src/database.ts` دعم متغيّر البيئة الاختياري `GARAGE_DB_PATH` (حين لا يوجد، السلوك الافتراضي دون أي تغيير). وُلّدت نسخة `scripts/preview-db/garage-simulation-preview.db` ببيانات الـ10 سنوات (اجتازت كل الفحوص). لمعاينة البيانات داخل التطبيق فعلياً:

```bash
GARAGE_DB_PATH=./scripts/preview-db/garage-simulation-preview.db npm run dev
```

راجع `scripts/README.md` لأوامر إعادة التوليد وضمانات العزل.

**ملاحظة:** تعذّر الاختبار اليدوي للواجهة الرسومية في بيئة التدقيق (لا شاشة عرض)؛ غُطّي منطق الـ backend كاملاً عبر المحاكاة على قنوات IPC الحقيقية.

---

## تحديث 2026-07-03 (٤) — دفعة إصلاحات وتحسينات ما بعد مراجعة المالك

سبعة بنود بعد معاينة بيانات محاكاة 10 سنوات داخل التطبيق. كل بند تُحقّق منه بـ `tsc --noEmit` + `vite build`؛ والبنود الخلفية (٣ و٦) تُحقّق أيضاً بتشغيل استعلاماتها الجديدة فعلياً على قاعدة المعاينة `garage-simulation-preview.db` عبر better-sqlite3 (بعد `npm rebuild` مؤقّت ثم إعادة البناء لـ Electron بـ `electron-rebuild`). لا مساس بأي منطق حسابي مالي قائم.

**١. علامة السالب على المجاميع/الفروق/الصافي السالبة فعلياً (`src/pages/CashLedger.tsx` + `Reports.tsx`):** المشكلة كانت أن دالة `fmt` في `CashLedger.tsx` تستخدم `Math.abs`، فأي **صافي/فرق/إجمالي نظام** سالب فعلاً يظهر بلون أحمر لكن **بلا علامة سالب** (يبدو كرقم موجب أحمر). أُضيفت دالة `fmtSigned(n) = (n<0?'−':'') + fmt(n)` وطُبِّقت على القيم القابلة للسالب فعلياً فقط: بطاقة «إجمالي النظام» وتفصيلها كاش/فيزا/شيك (صافي كل طريقة)، صفوف «النظام» في جدول مقارنة نهاية اليوم (لكل طريقة + الإجمالي)، عمود «الفرق» وتفصيله، عمود «إجمالي النظام» في سجل الإحصاءات ومودال التعديل ومودال الفرق والطباعة. **لم تُمَس** مبالغ «الصادر» الموجبة (مصاريف/رواتب/موردين) — تبقى حمراء بلا علامة سالب لأنها ليست سالبة (سلوك صحيح مقصود). في `Reports.tsx` دالة `fmt` لا تستخدم `Math.abs` أصلاً (السالب يظهر تلقائياً)، فقط لُوِّن الصافي السالب أحمر في جدولَي الشهري/السنوي.

**٢. سجل الإحصاءات اليومية — الضغط على صف يفلتر عليه (`CashLedger.tsx`):** صف جدول `daily_cash_audits` صار قابلاً للنقر (`mi-clickable-row`) ويحدّث `selectedDate` إلى تاريخ ذلك الصف — فتتحدّث تلقائياً البطاقات الخمس وسجل العمليات المرتبطة بـ`selectedDate` (بالمنطق الموجود أصلاً). أزرار طباعة/تعديل/حذف في نفس الصف مغلّفة بـ`onClick stopPropagation` كي لا تتعارض. لا مودال جديد. أُضيف سطر إرشادي أسفل الجدول.

**٣. صفحة الشيكات — زر «تفاصيل» (مودال) + زر «طباعة» (`src/pages/Cheques.tsx` + `src/db/cheques.ts`):** يبقى القرار التصميمي «لا تعديل/حذف من هنا» (أي شيك أصله فاتورة، والتعديل من صفحة الفاتورة الأصلية). المُضاف: مودال تفاصيل + إيصال طباعة (`printPdf`) يعرضان **كل معلومات العملية المصدر حسب `source`** — صيانة: اسم الزبون/الهاتف/نمرة السيارة/النوع/اللون/تاريخ الاستلام/التسليم/رقم الفاتورة/الإجمالي + تفاصيل الشيك؛ بيع مباشر: الزبون/الهاتف/رقم الفاتورة/الإجمالي/تاريخ البيع + الشيك؛ مورد/دين مورد: المورد/الهاتف/رقم الفاتورة/الإجمالي/تاريخ الشراء + الشيك. كل حقل يظهر فقط عند توفّره لمصدره (لا حقول وهمية). **وُسِّع `getAllCheques` فقط** (لا مساس بـ`getUpcomingCheques`) بأعمدة إضافية موجودة أصلاً في الجداول المصدر عبر `JOIN` (`invoice_number`, `party_phone`, `invoice_date`, `invoice_total`, `car_plate`, `car_type`, `car_color`, `date_released`) — بلا migration، الأعمدة موجودة أصلاً. أنواع: `ChequeRow` (DB) و`ChequeRecord` (UI) وُسِّعا، و`dbRowToCheque` حُدِّث. الصف صار قابلاً للنقر لفتح التفاصيل أيضاً. تحقّق فعلي على قاعدة المعاينة: 1832 شيكاً عبر الأربع مصادر، حقول الصيانة (نمرة/نوع/لون/إجمالي) والمورد مملوءة صحيحاً.

**٤. فلتر «دين / مدفوع» في فواتير الموردين (`src/pages/Suppliers.tsx`):** تبويبات `pd-type-tabs`/`pd-tab` (نفس نمط `PendingDebts.tsx`): «الكل» (افتراضي) / «دين» (`amountRemaining > 0.001`) / «مدفوع» (`amountRemaining ≤ 0.001`). فلترة client-side على `filteredSuppliers` القائمة، أُضيف `debtFilter` إلى `hasFilters`/`clearFilters` ومصفوفة اعتماديات `useMemo`. لا قناة IPC جديدة.

**٥. مكوّن `NameAutocomplete` المشترك (`src/components/NameAutocomplete.tsx`):** حقل نص + قائمة منسدلة تعرض الأسماء المطابقة (بعد `normalizeAr`) أثناء الكتابة، وكل الأسماء عند التركيز على حقل فارغ؛ اختيار اسم يملأ الحقل ويغلق القائمة (إغلاق أيضاً عند الضغط خارج المكوّن). أصناف CSS جديدة `name-ac-*` في `App.css`. مواضع الاستخدام: (أ) حقل «بحث باسم المورد» في فلاتر `Suppliers.tsx` — `options` = كل أسماء دليل الموردين. (ب) حقل «اسم المورد» في `SupplierInvoiceForm.tsx` — استبدل الـ`<select>` السابق؛ يحمّل قائمة الموردين مرة واحدة من `GarageContext` (`selectSupplier` يملأ الهاتف تلقائياً عند التطابق التام). **قناة `supplierInvoice:searchNames` كانت غير مستخدمة من الواجهة أصلاً** (النموذج كان `<select>` لا autocomplete حيّ) فتُركت كما هي دون استدعاء. **قرار تصميمي:** فلتر اختيار الموظف في `Employees.tsx` بقي `<select>` كما هو — يفلتر بـ`id` (لا اسم نصّي) وعدد الموظفين قليل ومُدار بسهولة، وهو الخيار الذي أجازه المالك صراحةً للقوائم الصغيرة.

**٦. تقارير شهري/سنوي أشمل (`src/db/reports.ts` + `src/pages/Reports.tsx`):** أُضيفت 13 حقلاً لنتيجة `report:monthly` القائمة (بلا قناة جديدة، بنفس فلسفة تحديث (١٣)): دخل الصيانة، دخل البيع المباشر، المصاريف اليومية، الرواتب (الصافي)، مدفوعات الموردين (شراء + سداد ديون)، الديون المُحصَّلة، الديون الجديدة، عدد فواتير الصيانة/البيع/المشتريات، خصومات الفواتير، خصومات التسوية، عدد الكفالات الجديدة. **مصدر الحقيقة:** المجاميع المالية من نفس `cash_ledger` المجمّع حسب `reference_type` للشهر (نفس منطق `getDailyReport`)؛ الأعداد والديون الجديدة والخصومات والكفالات من الجداول المصدر مباشرة حسب تاريخ الفاتورة (`date_received`/`sale_date`/`purchase_date`/`start_date`). **تعريفات موثّقة:** «الديون الجديدة» = مجموع المتبقّي الحالي للفواتير التي أُنشئت في الفترة (قابل للانخفاض إن حُصِّل لاحقاً)؛ «خصومات الفواتير» = مجموع البنود قبل الخصم − الإجمالي بعد الخصم (للفواتير ذات خصم فقط)؛ «الديون المُحصَّلة» = `debt_customer` من الصندوق. التبويب السنوي يجمع 13 حقلاً من الأشهر الاثني عشر في `Reports.tsx`. **العرض:** بطاقات `stat-card` مجمّعة في 4 مجموعات معنونة (الإيرادات/المصروفات/الديون والخصومات/الأعداد) لتبويبي شهري وسنوي؛ الطباعة (`printPdf`) تضمّ نفس الأرقام؛ تصدير CSV و Excel يُلحِقان «ملخّص الفترة» كصفوف label/value بعد جدول السلسلة الزمنية. تحقّق فعلي على قاعدة المعاينة: كل الاستعلامات تُنفَّذ وتُعيد أرقاماً سليمة.

**٧. أقسام قابلة للطي (`src/components/CollapsibleCard.tsx`):** مكوّن مشترك يلفّ `mi-card` بعنوان قابل للطي/الفرد (سهم ▾/▸)، مبدئياً مفتوح (لا تغيير على السلوك الافتراضي)، مع `headerRight` اختياري لإبقاء زر «+ إضافة» في الترويسة. أصناف CSS جديدة `mi-collapse-*` في `App.css`. طُبِّق على كل الصفحات ذات الأقسام المتعددة المعروضة معاً: `MaintenanceInvoices.tsx` (قيد الصيانة / تم التسليم)، `Warranties.tsx` (سارية / منتهية)، `CashLedger.tsx` (سجل العمليات / سجل الإحصاءات اليومية)، `Suppliers.tsx` (قائمة الموردين / فواتير الموردين)، `Employees.tsx` (قائمة الموظفين / سجل الرواتب). **لم يُطبَّق** على الصفحات ذات القسم الواحد (`DirectSales`/`PendingDebts`/`SalesInvoices`/`PurchaseInvoices`/`DailyExpenses`) ولا على `Reports.tsx` (كل تبويب تقرير واحد متماسك، وتبويب الديون جدولاه جزءان من تقرير واحد) — تطبيقاً لقاعدة «لا طي لصفحة فيها قسم واحد فقط».

**ملاحظة توثيق (صفحة الشيكات):** «قراءة فقط» في `Cheques.tsx` تعني الآن تحديداً **لا تعديل ولا حذف من هذه الصفحة** (يبقيان حصراً من صفحة الفاتورة الأصلية)، بينما **التفاصيل والطباعة مضافتان** كعرض قراءة فقط.

**ملاحظة:** تعذّر الاختبار اليدوي للواجهة الرسومية (لا شاشة عرض في البيئة)؛ غُطّيت الواجهة عبر typecheck + build، والخلفية عبر تنفيذ الاستعلامات الجديدة فعلياً على قاعدة المعاينة.
